...
//角速度
union FloatToBytes angular;

uint8_t  usart1_receive_data[10];
uint16_t received_data_index=0;
uint16_t FLAG_ROS=0;

void Process_received_data(uint8_t *data_);
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	{ 
			_DATA = USART_ReceiveData(USART1);
		// 寻找数据包开头
		if(_DATA == 0x5a)
			flag = 1;
		else if(_DATA == 0xa5){
			flag = 0;
			usart1_receive_data[received_data_index++] = _DATA;
		}
		if(flag)
		{
			usart1_receive_data[received_data_index++] = _DATA;
		...
		//数据接受完成
		if(received_data_index == 10)
		{ //判断包是否有效
			if(usart1_receive_data[0] == 0x5a && usart1_receive_data[9] == 0xa5)
			{
				//处理数据
				Process_received_data(usart1_receive_data);
				//清空数据包，方便下次重新接受数据
				memset(usart1_receive_data ,0,sizeof (usart1_receive_data ));
...
void Process_received_data(uint8_t *data_)
{
	FLAG_ROS=1;//数据包正确且接受完成
	
	linear.bytes[0] = data_[1];
	linear.bytes[1] = data_[2];
	linear.bytes[2] = data_[3];
	linear.bytes[3] = data_[4];
	
	angular.bytes[0] = data_[5];
	angular.bytes[1] = data_[6];
	angular.bytes[2] = data_[7];
	angular.bytes[3] = data_[8];
	
	_chassis.l_v = ((30*19)/(3.14*0.07))*linear.value ;
	_chassis.w_v = angular.value *66 ;
